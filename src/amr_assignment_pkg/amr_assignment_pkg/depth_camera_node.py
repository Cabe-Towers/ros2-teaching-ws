...
class DepthCameraNode(Node):
    def __init__(self):
        ...
        # Stores the location of the red and green arena markers, uses depth_link frame
        self.green_marker_point = None
        self.red_marker_point = None

        # Stores the location of detected boxes, uses depth_link frame
        self.green_box_points = []
        self.red_box_points = []

        # Store depth and image data with timestamps for accurate transforms
    ...
    # Returns the location of visible markers in the depth_link frame
    ...
    # Returns the locations of visible boxes, uses depth_link frame
    ...
    # Returns false if given point is around the edge of the image frame, used to remove possibly invalid locations with partial visibility
    ...
    # Processes box and marker locations with depth data when an image is received
    def camera_callback(self, data: Image):
        try:
            cv_image = self.br.imgmsg_to_cv2(data, "bgr8")
        except CvBridgeError as e:
            print(e)
        # Convert image to opencv format and apply threshold
        ...
        # Init timestamp and local variables
        ...
        for c in hsv_contours:
            M = cv2.moments(c) # Gets center of each contour
            if M['m00'] != 0:
                # Coords of center of contour
                cx = int(M['m10']/M['m00'])
                cy = int(M['m01']/M['m00'])
                if self.depth_data is not None and self.check_camera_exclusion(cx, cy): # Check for valid depth data and fully visible
                    x,y,z = self.point_from_pointcloud(cx,cy) # Get 3d location of point from pointcloud
    ...
    # Converts points to numpy array and stores the data
    ...
    # Convert pixel xy in image to 3d pointcloud location
